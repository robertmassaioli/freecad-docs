...
| Curvilinear equivalence | Checkbox | On | (Auxiliary mode only) Compute orientation by placing equidistant points on both spines. Uncheck to use parallel projection instead. |
...
| X / Y / Z | Float | 0.0 | (Binormal mode only) Components of the fixed binormal vector used to compute the profile's normal at each point. |

**Orientation modes:**

| Mode | Behaviour |
|------|-----------|
| **Standard** | FreeCAD chooses orientation automatically using the Darboux frame (minimum rotation). Best default for most paths. |
| **Fixed** | The profile's orientation in world space stays exactly as drawn. Useful for profiles that must stay upright (e.g. a square slot that must never rotate). |
...
| **Auxiliary** | Uses a second spine sketch to drive the roll of the profile. Most complex but most controllable for twisted sweeps. |
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| Transform mode | Dropdown | Constant | `Constant`: the profile shape is identical at every point. `Multisection`: the profile morphs between a list of intermediate section sketches placed along the path. `Linear`, `S-shape`, `Interpolation`: additional mathematical interpolation modes. <!-- TODO: verify exact UI labels for Linear/S-shape/Interpolation --> |
...
| Section list | Ordered list | — | (Multisection only) The intermediate sections in path order. All intermediate sections must have the same number of closed wires as the start profile; only the final section may be a single point. |
